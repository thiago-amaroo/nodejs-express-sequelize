...
//isso acima é a mesma coisa que this.pessoaServices = new Services('Pessoa');

class CursoServices extends Services {
  constructor() {
    super('Curso');
    this.pessoaServices = new Services('Pessoa');
  }
  ...
  async pegaEstudantesMatriculadosDoCurso (cursoId) {
    ...
    const listaEstudantes = await Promise.all(listaMatriculas.map(async (elemento) => await this.pessoaServices.pegaRegistrosPorEscopoEId('todosOsRegistros', elemento.estudante_id )));
